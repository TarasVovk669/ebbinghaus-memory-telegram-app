...
public class TelegramBotServiceImpl implements TelegramBotService {
    ...
    private final Function<InputUserData, Boolean> handleQuizNextQuestion = userData -> {
    ...
    private final Function<InputUserData, Boolean> handleStartMessage = userData -> {
        ...
        Message photoMessage = telegramClientService.sendPhotoMessage(
                userData.getChatId(),
                String.format(
                        messageSourceService.getMessage("messages.greeting.start", userData.getLanguageCode()),
        ...
        telegramClientService.sendMessage(
                userData.getChatId(),
                String.format(
                        messageSourceService.getMessage("messages.greeting.help", userData.getLanguageCode()),
                        userData.getUser().getFirstName()));
    ...
    private final Function<InputUserData, Boolean> handleMessageView = userData -> {
        messageService
                .getMessageOptional(Long.valueOf(userData.getCallBackData().get(MESSAGE_ID)), true)
                .ifPresentOrElse(
                        message -> {
                            var suffix = messageSourceService.getMessage(
    ...
    private final Function<InputUserData, Boolean> handleMessageDelete = userData -> {
        ...
        telegramClientService.sendEditMessage(
                ...
                MessageDataRequest.builder()
                        ...
                        .messageId(userData.getMessageId())
                        .entities(List.of())
                        .replyKeyboard(keyboardService.getDeleteKeyboard(messageId, userData.getLanguageCode()))
                        .file(userData.getFile())
                        .build());
        return Boolean.TRUE;
    };
    private final Function<InputUserData, Boolean> handleMessageRestart = userData -> {
        telegramClientService.sendEditMessage(
                userData.getMessageType(),
                MessageDataRequest.builder()
                        .chatId(userData.getChatId())
                        ...
                        .messageId(userData.getMessageId())
                        .entities(List.of())
                        .replyKeyboard(keyboardService.getRestartKeyboard(
                                Long.valueOf(userData.getCallBackData().get(MESSAGE_ID)), userData.getLanguageCode()))
                        .file(userData.getFile())
                        .build());
        return Boolean.TRUE;
    };
    private final Function<InputUserData, Boolean> handleTestMessage = userData -> {
        telegramClientService.sendEditMessage(
                userData.getMessageType(),
                MessageDataRequest.builder()
                        .chatId(userData.getChatId())
                        .messageText(messageSourceService.getMessage("messages.test.init", userData.getLanguageCode()))
                        .messageId(userData.getMessageId())
                        .build());
    ...
    private final Function<InputUserData, Boolean> handleFullMessageBack = userData -> {
        var message = messageService.getMessage(
                Long.valueOf(userData.getCallBackData().get(MESSAGE_ID)), true);
        sendMessageBack(userData, message, true);
        return Boolean.TRUE;
    };
    private final Function<InputUserData, Boolean> handleMessageDeleteYes = userData -> {
        messageService
    ...
    private final Function<InputUserData, Boolean> handleRemainderQuizNo = userData -> {
        telegramClientService.deleteMessage(userData.getChatId(), userData.getMessageId(), userData.getLanguageCode());

        return Boolean.TRUE;
    };
    private final Function<InputUserData, Boolean> handleQuizPushDisable = userData -> {
        userService.changeQuizRemainderEnabled(userData.getUser().getId(), false);

        telegramClientService.sendEditMessage(
                userData.getChatId(),
                String.format(
                        messageSourceService.getMessage(
                                "messages.profile.disable-quiz-notifications.response", userData.getLanguageCode()),
                        userData.getUser().getFirstName()),
    ...
    private final Function<InputUserData, Boolean> handleQuizPushEnable = userData -> {
        userService.changeQuizRemainderEnabled(userData.getUser().getId(), true);

        telegramClientService.sendEditMessage(
                userData.getChatId(),
                String.format(
                        messageSourceService.getMessage(
                                "messages.profile.activate-quiz-notifications.response", userData.getLanguageCode()),
                        userData.getUser().getFirstName()),
                keyboardService.getSingleBackProfileKeyboard(userData.getLanguageCode()),
                null,
                userData.getMessageId());
    ...
    private final Function<InputUserData, Boolean> handleMessageRestartYes = userData -> {
        var message = messageService.restartMessageAndSchedule(
                Long.valueOf(userData.getCallBackData().get(MESSAGE_ID)), userData.getChatId());
        sendMessageBack(userData, message, false);
        return Boolean.TRUE;
    };
    private final Function<InputUserData, Boolean> handleMessageNoAction = userData -> {
        var message = messageService.getMessage(
                Long.valueOf(userData.getCallBackData().get(MESSAGE_ID)), true);
    ...
    private final Function<InputUserData, Boolean> handleMessageChangeLanguage = userData -> {
        var user = userService.getUser(userData.getUser().getId());

        telegramClientService.sendEditMessage(
                userData.getChatId(),
                String.format(
                        messageSourceService.getMessage("messages.profile.language", user.getLanguageCode()),
    ...
    private final Function<InputUserData, Boolean> howItWorksCallback = userData -> {
        telegramClientService.sendEditMessage(
                userData.getChatId(),
                String.format(
    ...
    private final Function<InputUserData, Boolean> handleProfileMainMenuBack = userData -> {
        var messageAndCategoryCountCF = CompletableFuture.supplyAsync(
                () -> messageService.getMessageAndCategoryCount(
                        userData.getUser().getId()),
                virtualTaskExecutor);

        var quizzesCountCF = CompletableFuture.supplyAsync(
        ...
        CompletableFuture.allOf(messageAndCategoryCountCF, quizzesCountCF, audioCountCF)
                ...
                .thenAcceptAsync(
                        text -> telegramClientService.sendEditMessage(
                                userData.getChatId(),
                                text,
                                keyboardService.getProfileKeyboard(quizRemainderCF.join(), userData.getLanguageCode()),
                                null,
                                userData.getMessageId()),
                        virtualTaskExecutor);
        return Boolean.TRUE;
    };
    private final Function<InputUserData, Boolean> handleChangeLanguage = userData -> {
    ...
    private final Function<InputUserData, Boolean> handleContactInfo = userData -> {
        var text = String.format(
                messageSourceService.getMessage("messages.profile.contact-info.text", userData.getLanguageCode()),
    ...
    private final Function<InputUserData, Boolean> handleTextToSpeech = userData -> Optional.of(userData)
            .filter(u -> audioService.canGenerate(userData.getUser().getId()))
            .map(u -> {
                var initAudioMessage = telegramClientService.sendMessage(
                        userData.getChatId(),
                        messageSourceService.getMessage(
                ...
                virtualTaskExecutor.execute(() -> {
                    ...
                    try {
                        var bytes = ttsService.synthesize(messageService
                                .getMessage(
                                        Long.valueOf(userData.getCallBackData().get(MESSAGE_ID)), false)
                                .getText());
                        telegramClientService.deleteMessage(u.getChatId(), initAudioMessage.getMessageId());

                        var audioMessage = telegramClientService.sendAudioMessage(
            ...
            .orElseGet(() -> {
                doTry((() -> telegramClientService.sendEditMessage(EditMessageText.builder()
                        .chatId(userData.getChatId())
                        .messageId(userData.getMessageId())
                        .text(messageSourceService.getMessage("messages.error.audio.limit", userData.getLanguageCode()))
    ...
    private final Function<InputUserData, Boolean> handleHelpMessage = userData -> {
        ...
        chatMessageStateService.addMessage(
                userData.getUser().getId(), userData.getChatId(), UserState.HELP, List.of(message.getMessageId()));

        return Boolean.TRUE;
    };
    private final Function<InputUserData, Boolean> handleButtonAddNewInfo = userData -> {
        clearMessages(userData, List.of(WAIT_TEXT, WAIT_EDIT_TEXT_CONCRETE));
        telegramClientService.deleteMessage(userData.getChatId(), userData.getMessageId());

        var message = telegramClientService.sendMessage(
                userData.getChatId(),
                messageSourceService.getMessage("messages.input.waiting-data", userData.getLanguageCode()));
        chatMessageStateService.addMessage(
                userData.getUser().getId(), userData.getChatId(), WAIT_TEXT, List.of(message.getMessageId()));

        userService.setUserState(userData.getUser().getId(), WAIT_TEXT);
        return Boolean.TRUE;
    };
    private final Function<InputUserData, Boolean> handleButtonInfoList = userData -> {
        ...
        if (messages.getTotalElements() == ZERO_COUNT) {
            ...
            var dataInfoEmptyMessage = telegramClientService.sendMessage(
                    userData.getChatId(),
                    messageSourceService.getMessage("messages.collection.empty", userData.getLanguageCode()));
        ...
        var result = new StringBuilder(titleListString(
                page,
                size,
                messages.getTotalElements(),
                messageSourceService.getMessage("messages.list.title", userData.getLanguageCode())));
        var buttons = new ArrayList<InlineKeyboardButton>();

        var entities = new ArrayList<MessageEntity>(List.of(MessageEntity.builder()
                .type(BOLD_STYLE)
                .offset(0)
        ...
        messages.forEach(m -> {
            ...
            Optional.ofNullable(m.getFile()).ifPresent(file -> {
                var startFile = result.length();

                result.append(messageSourceService.getMessage(
                        "file.type.".concat(file.getFileType().name().toLowerCase()), userData.getLanguageCode()));
            ...
            var list = getMessageEntities(m.getMessageEntities(), msgString.length(), objectMapper);
            entities.addAll(list.stream()
            ...
            buttons.add(InlineKeyboardButton.builder()
                    ...
                    .callbackData(doTry(() -> objectMapper.writeValueAsString(Map.ofEntries(
                            Map.entry(OPERATION, VIEW_SHORT_MESSAGE_CALLBACK), Map.entry(MESSAGE_ID, m.getId())))))
                    .build());
        });

        var navigationButtons =
                keyboardService.getNavigationButtons(page, size, messages, NAVIGATION_DATA_LIST_CALLBACK, userData);

        Optional.ofNullable(userData.getCallBackData())
                .ifPresentOrElse(
                        cd -> telegramClientService.sendEditMessage(
                                userData.getChatId(),
                                result.toString(),
                                new InlineKeyboardMarkup(List.of(
                                        new InlineKeyboardRow(buttons), new InlineKeyboardRow(navigationButtons))),
                                entities,
                                userData.getMessageId()),
                        () -> {
                            telegramClientService.deleteMessage(userData.getChatId(), userData.getMessageId());
                            ...
                            var msg = telegramClientService.sendMessage(
                                    userData.getChatId(),
                                    result.toString(),
                                    new InlineKeyboardMarkup(List.of(
                                            new InlineKeyboardRow(buttons), new InlineKeyboardRow(navigationButtons))),
    ...
    private final Function<InputUserData, Boolean> handleButtonCategoryList = userData -> {
        ...
        var categories =
                categoryService.getCategories(userData.getUser().getId(), page, size, Sort.by(Sort.Order.desc("id")));

        if (categories.getTotalElements() == ZERO_COUNT) {
            telegramClientService.deleteMessage(userData.getChatId(), userData.getMessageId());
            clearMessages(userData, CATEGORY_DATA_LIST);

            var emptyInfoMessage = telegramClientService.sendMessage(
                    userData.getChatId(),
                    messageSourceService.getMessage("messages.collection.category.empty", userData.getLanguageCode()));
        ...
        var navigationButtons = keyboardService.getNavigationButtons(
    ...
    private final Function<InputUserData, Boolean> handleProfileMainMenu = userData -> {
                        ...
                        text -> {
                            var message = telegramClientService.sendMessage(
                                    ...
                                    keyboardService.getProfileKeyboard(
                                            quizRemainderCF.join(), userData.getLanguageCode()));
    ...
    private final Function<InputUserData, Boolean> handleShortMessageView = userData -> {
                        ...
                        message -> {
                            ...
                            var msg = telegramClientService.sendMessage(
                                    ...
                                    MessageDataRequest.builder()
                                            .chatId(userData.getChatId())
                                            .messageText(messageString)
                                            .entities(manageMessageEntitiesShortMessage(
                                                    message.getMessageEntities(),
                                                    messageString,
                                                    SHORT_MESSAGE_SYMBOL_QUANTITY,
                                                    suffix,
                                                    objectMapper))
                                            .replyKeyboard(keyboardService.getMessageKeyboard(
    ...
    private final Function<InputUserData, Boolean> handleMessageEdit = userData -> {
        ...
        chatMessageStateService.addMessage(
                userData.getUser().getId(), userData.getChatId(), WAIT_EDIT_TEXT, List.of(msg.getMessageId()));
        return Boolean.TRUE;
    };
    private final Function<InputUserData, Boolean> handleEditConcreteMessage = userData -> {
        clearMessages(userData, List.of(WAIT_EDIT_TEXT, WAIT_EDIT_TEXT_CONCRETE));
        var editedMessage = messageService.updateMessage(prepareEditedMessage(
        ...
        var suffix = messageSourceService.getMessage("messages.suffix.execution-time", userData.getLanguageCode());
        var messageString =
                parseMessage(editedMessage, false, suffix, userData.getLanguageCode(), messageSourceService);
        ...
        telegramClientService.sendMessage(
                ...
                MessageDataRequest.builder()
                        ...
                        .entities(manageMessageEntitiesShortMessage(
                                ...
                                objectMapper))
                        .replyKeyboard(
                                keyboardService.getMessageKeyboard(editedMessage.getId(), userData.getLanguageCode()))
    ...
    private final Function<InputUserData, Boolean> handleInputText = userData -> {
        if (null != userData.getMessageText()
            ...
            telegramClientService.sendMessage(
                    userData.getChatId(),
                    messageSourceService.getMessage("messages.error.length-allowed", userData.getLanguageCode()));
        ...
        var suffix = messageSourceService.getMessage("messages.suffix.execution-time", userData.getLanguageCode());
        var messageString = parseMessage(message, false, suffix, userData.getLanguageCode(), messageSourceService);
        ...
        telegramClientService.sendMessage(
                ...
                MessageDataRequest.builder()
                        ...
                        .entities(manageMessageEntitiesShortMessage(
                                ...
                                suffix,
                                objectMapper))
                        .replyKeyboard(keyboardService.getMessageKeyboard(message.getId(), userData.getLanguageCode()))
                        .file(message.getFile())
                        .build());
    ...
    public TelegramBotServiceImpl(
        ...
        functionCallbackDataMap.put(PUSH_QUIZ_ENABLE, handleQuizPushEnable);
        functionCallbackDataMap.put(PUSH_QUIZ_DISABLE, handleQuizPushDisable);
    ...
    public void processInputCallback(InputUserData inputUserData) {
        Optional.ofNullable(inputUserData.getMessageText())
                .ifPresentOrElse(
                        messageText -> Optional.ofNullable(
                                        messageText.startsWith("/")
                                                ? functionCommandMap.get(inputUserData.getMessageText()
                                                        .split(" ")[0])
                                                : functionCommandMap.get(
                                                        extractSubstringForButton(inputUserData.getMessageText())))
                                .ifPresentOrElse(function -> function.apply(inputUserData), () -> Optional.ofNullable(
                                                functionUserStateMap.get(inputUserData.getState()))
                                        .ifPresentOrElse(
    ...
    private MessageTuple prepareEditedMessage(EMessage eMessage, InputUserData userData) {
        eMessage.setText(userData.getMessageText())
                .setFile(userData.getFile())
                .setMessageEntities(Optional.ofNullable(userData.getMessageEntities())
    ...
    private MessageTuple parseInput(InputUserData userData) {
        ...
        return new MessageTuple(
                EMessage.builder()
                        .text(input)
                        .messageEntities(Optional.ofNullable(userData.getMessageEntities())
                                .map(entities -> entities.stream()
                                        .map(me -> {
                                            try {
                                                return EMessageEntity.builder()
    ...
    public void clearMessages(InputUserData userData, List<UserState> statuses) {
        statuses.forEach(s -> {
            var messages =
                    chatMessageStateService.getMessages(userData.getUser().getId(), userData.getChatId(), s);
            if (!messages.isEmpty()) {
    ...
    private void sendMessageBack(InputUserData userData, EMessage message, boolean isFull) {
        var suffix = messageSourceService.getMessage("messages.suffix.execution-time", userData.getLanguageCode());
        var messageString = parseMessage(message, isFull, suffix, userData.getLanguageCode(), messageSourceService);
    ...
    private void manageInvalidInputMessage(InputUserData inputUserData) {
        String key = inputUserData
                .getUser()
                .getId()
                .toString()
                .concat(inputUserData.getChatId().toString());
